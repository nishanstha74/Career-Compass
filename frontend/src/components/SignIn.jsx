...
export default function SignIn() {
  ...
  // "signin" | "forgot-request" | "forgot-verify" | "forgot-reset"
  ...
  // ---------- Remember Me: prefill email on mount ----------
  useEffect(() => {
  ...
  const handleSendResetCode = async (e) => {
    ...
    try {
      ...
      setResetError(
        err.errors?.[0]?.message || "Couldn't send reset code. Try again.",
      );
    } finally {
      setResetLoading(false);
    }
  ...
  const handleVerifyResetCode = async (e) => {
    ...
    try {
      ...
      if (result.status === "needs_new_password") {
        ...
        setResetError("Something unexpected happened. Please try again.");
      }
    } catch (err) {
      console.error("Verify code error:", err);
      setResetError(err.errors?.[0]?.message || "Invalid or expired code.");
    } finally {
      setResetLoading(false);
    }
  };
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setResetError(
        err.errors?.[0]?.message || "Couldn't reset password. Try again.",
      );
    } finally {
      setResetLoading(false);
    }
